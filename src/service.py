...
class BaseService:
    ...
    async def get_all(
        ...
        **filter_by,
        ...
        return await cls.repository.get_all(
            session, limit, offset, order_by, **filter_by
        )

    @classmethod
    async def get_max_page(
        cls, session: AsyncSession, limit: int = 10, **filter_by
    ) -> int:
    ...
    async def create_all(
        cls, data: list[BaseCreate], session: AsyncSession
    ) -> list[Base]:
        return await cls.repository.create_all(data, session)

    @classmethod
    async def update(
        cls, id: uuid.UUID, data: BaseUpdate, session: AsyncSession
    ) -> Base:
