...
async def send_report_of_scanning(result: BaseCeleryTaskReslt, data: Optional[BaseCeleryTaskData] = None):
    media = []
    ...
    async with container() as c:
        ...
        if data:
            report_text += ""
            try:
                message = await bot.send_message(chat_id=data.initiator_id, text=report_text)
            except Exception as e:
                logger.error(f'Ошибка отправки отчета инициатору {data.initiator_id}: {e}')
        else:
            report_text += ""
...
async def _send_user_report(bot: Bot, user_id: int, report_text: str, media: list) -> None:
    try:
